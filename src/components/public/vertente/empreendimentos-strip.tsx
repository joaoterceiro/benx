...
export function EmpreendimentosStrip({
  ...
  return (
    <div className="relative" onMouseEnter={() => { pausado.current = true; }} onMouseLeave={() => { pausado.current = false; }}>
      ...
      <div ref={ref} className="flex snap-x gap-4 overflow-x-auto scroll-smooth px-4 scroll-px-4 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        {cards.map((c) => (
          <Link
            key={c.href}
            href={c.href}
